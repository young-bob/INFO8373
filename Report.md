...
**Client Framework:** Avalonia UI 12.0 (Cross-Platform Desktop)  
**Database:** SQLite / Entity Framework Core (EF Core)  
**File Storage:** Local File System (`VaultStorage` isolated directory)  

**Justification:** 
The .NET 10 framework was selected because of its hardened security features. ASP.NET Core Minimal APIs enforce strict routing and binding, minimizing attack surfaces. EF Core automatically uses parameterized queries to entirely neutralize SQL Injection (SQLi) attacks. We chose Avalonia UI for the client to completely avoid browser-based DOM vulnerabilities (like DOM-based XSS) while maintaining native OS-level integration. All authentication leverages HttpOnly secure cookies managed by `CookieContainer`, neutralizing standard token theft via XSS.
...
```mermaid
graph TD
    Client[Avalonia Desktop Client]
    AppServer[ASP.NET Core Server]
    Auth[Authentication Service]
    FileSys[(VaultStorage - AES-256)]
    DB[(SQLite - EF Core)]

    Client -- "1. Login (TLS)" --> Auth
    Auth -- "2. Return Session Cookie (HttpOnly)" --> Client
    
    Client -- "3. Upload Stream (TLS)" --> AppServer
    AppServer -- "4. Insert Meta (Parameterized)" --> DB
    AppServer -- "5. Encrypt On-the-Fly (AES-256)" --> FileSys
    
    Client -- "6. Access Share Link (?pwd=xxx)" --> AppServer
    AppServer -- "7. Verify IDOR & Expiry" --> DB
    AppServer -- "8. Decrypt on-the-fly" --> FileSys
    FileSys -- "Stream" --> Client
```
...
Upon successful login, a JWT is securely returned. By utilizing `CookieContainer` on the generic desktop configuration, it transports cookies securely. The backend sets the `HttpOnly` and `Secure` flags, meaning malicious code cannot read them, forcing all API requests to carry the credential securely.
...
The application redirects HTTP to HTTPS. All desktop-to-API transmission operates inside a TLS encrypted tunnel, circumventing Man-In-The-Middle sniffing. The `SecurityHeadersMiddleware.cs` intercepts requests to inject `X-Content-Type-Options: nosniff` and CSP headers.
...
Debug pages and stack traces are suppressed in Production, replacing `DeveloperExceptionPage` with generic JSON error responses in `GlobalExceptionHandlerMiddleware.cs`, denying attackers footprinting info.
...
**Data to Backup:** SQLite database file, the `VaultStorage` directory, and `.env` secrets.
**Policy (3-2-1 Rule):** 3 copies of data, across 2 different formats/media, with 1 copy off-site (cloud block storage).
**RTO / RPO:** 
- **RTO (Recovery Time Objective):** 4 Hours to spin up new infrastructure.
- **RPO (Recovery Point Objective):** 1 Hour (Incremental backup frequencies).
**Recovery Steps:** Replace physical hardware -> Provision OS -> Restore `.env` -> Inject `VaultStorage` volume -> Start .NET Services.
